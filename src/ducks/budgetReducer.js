import axios from 'axios';

const initialState = {
    purchases: [],
    budgetLimit: null,
    loading: false
}

const REQUEST_BUDGET_DATA = 'REQUEST_BUDGET_DATA';
const ADD_PURCHASE = 'ADD_PURCHASE';
const REMOVE_PURCHASE = 'REMOVE_PURCHASE';

export function removePurchase (id) {
    let object = axios.delete(`/api/budget-data/purchase/${id}`)
        .then( res => {
            return res.data
        })
    return {
        type: REMOVE_PURCHASE,
        payload: object
    }
}

export function addPurchase (price, description, category) {
    let object = axios.post('/api/budget-data/purchase', {description: description, price: price, category: category})
        .then(res => {
            return res.data
        })
    return {
        type: ADD_PURCHASE,
        payload: object
    }
}

export function getBudgetData () {
    let budgetData = axios.get('/api/budget-data').then(res => {
        return res.data
    })
    console.log(budgetData)
    return {
        type: REQUEST_BUDGET_DATA,
        payload: budgetData
    }
}

export default function reducer(state = initialState, action) {
    switch (action.type) {
        case REQUEST_BUDGET_DATA + '_PENDING':
            return { ...state, loading: true }

        case REQUEST_BUDGET_DATA + '_FULFILLED':
            return {
                purchases: action.payload.purchases,
                budgetLimit: action.payload.budgetLimit,
                loading: false
            }

        case ADD_PURCHASE + '_PENDING':
            return { ...state, loading: true }

        case ADD_PURCHASE + '_FULFILLED':
            return {
                ...state,
                purchases: action.payload,
                loading: false
            }

        case REMOVE_PURCHASE + '_PENDING':
            return { ...state, loading: true }

        case REMOVE_PURCHASE + '_FULFILLED':
            return {
                ...state,
                purchases: action.payload,
                loading: false
            }

        default:
            return state
    }
}